...
export function scoreCommunity(data) {
    ...
    const contributorPoints = (() => {
        ...
        if (c >= COMMUNITY.CONTRIBUTORS_EXCELLENT)
            return COMMUNITY.CONTRIBUTORS_MAX_POINTS;
        ...
        if (c === 1)
            return Math.round(COMMUNITY.CONTRIBUTORS_MAX_POINTS * 0.1);
        return 0;
    })();
    ...
    const growthPoints = (() => {
        ...
        if (g >= COMMUNITY.GROWTH_EXCELLENT_PCT)
            return COMMUNITY.GROWTH_MAX_POINTS;
    ...
    const newContribPoints = (() => {
        ...
        if (n >= COMMUNITY.NEW_CONTRIBUTORS_EXCELLENT)
            return COMMUNITY.NEW_CONTRIBUTORS_MAX_POINTS;
