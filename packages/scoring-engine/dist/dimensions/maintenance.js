 ...
 */
function interpolate(value, minVal, maxVal, maxScore, minScore = 0, invert = false) {
...
export function scoreMaintenance(data) {
    const breakdown = [];
    let total = 0;
    ...
    const issueResponsePoints = (() => {
        ...
        if (h <= MAINTENANCE.ISSUE_RESPONSE_EXCELLENT_HOURS) {
            ...
            return interpolate(h, MAINTENANCE.ISSUE_RESPONSE_EXCELLENT_HOURS, MAINTENANCE.ISSUE_RESPONSE_GOOD_HOURS, MAINTENANCE.ISSUE_RESPONSE_MAX_POINTS, Math.round(MAINTENANCE.ISSUE_RESPONSE_MAX_POINTS * 0.6), true);
        }
        else if (h <= MAINTENANCE.ISSUE_RESPONSE_POOR_HOURS) {
            return interpolate(h, MAINTENANCE.ISSUE_RESPONSE_GOOD_HOURS, MAINTENANCE.ISSUE_RESPONSE_POOR_HOURS, Math.round(MAINTENANCE.ISSUE_RESPONSE_MAX_POINTS * 0.6), Math.round(MAINTENANCE.ISSUE_RESPONSE_MAX_POINTS * 0.2), true);
        }
    ...
    const prReviewPoints = (() => {
        ...
        if (h <= MAINTENANCE.PR_REVIEW_EXCELLENT_HOURS) {
            ...
            return interpolate(h, MAINTENANCE.PR_REVIEW_EXCELLENT_HOURS, MAINTENANCE.PR_REVIEW_GOOD_HOURS, MAINTENANCE.PR_REVIEW_MAX_POINTS, Math.round(MAINTENANCE.PR_REVIEW_MAX_POINTS * 0.6), true);
        }
        else if (h <= MAINTENANCE.PR_REVIEW_POOR_HOURS) {
            return interpolate(h, MAINTENANCE.PR_REVIEW_GOOD_HOURS, MAINTENANCE.PR_REVIEW_POOR_HOURS, Math.round(MAINTENANCE.PR_REVIEW_MAX_POINTS * 0.6), Math.round(MAINTENANCE.PR_REVIEW_MAX_POINTS * 0.2), true);
        }
    ...
    const releaseRecencyPoints = (() => {
        ...
        if (d <= MAINTENANCE.RELEASE_RECENCY_EXCELLENT_DAYS)
            return MAINTENANCE.RELEASE_RECENCY_MAX_POINTS;
        if (d <= MAINTENANCE.RELEASE_RECENCY_GOOD_DAYS) {
            return interpolate(d, MAINTENANCE.RELEASE_RECENCY_EXCELLENT_DAYS, MAINTENANCE.RELEASE_RECENCY_GOOD_DAYS, MAINTENANCE.RELEASE_RECENCY_MAX_POINTS, Math.round(MAINTENANCE.RELEASE_RECENCY_MAX_POINTS * 0.5), true);
        }
        if (d <= MAINTENANCE.RELEASE_RECENCY_POOR_DAYS) {
            return interpolate(d, MAINTENANCE.RELEASE_RECENCY_GOOD_DAYS, MAINTENANCE.RELEASE_RECENCY_POOR_DAYS, Math.round(MAINTENANCE.RELEASE_RECENCY_MAX_POINTS * 0.5), 0, true);
        }
        return 0;
    })();
