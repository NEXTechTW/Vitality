...
export function scoreSecurity(data) {
    const breakdown = [];
    ...
    if (data.outdatedDependencies > 0) {
        const outdatedPenalty = Math.min(data.outdatedDependencies * SECURITY.OUTDATED_DEPS_PENALTY_PER_DEP, SECURITY.OUTDATED_DEPS_MAX_PENALTY);
    ...
    if (secReleaseDays !== Infinity) {
        const secReleasePoints = (() => {
            if (secReleaseDays <= SECURITY.SEC_RELEASE_EXCELLENT_DAYS)
                return SECURITY.SEC_RELEASE_MAX_POINTS;
