...
import type { NormalizedRepoData } from '../types.js';
...
const thriving = thrivingFixture as unknown as NormalizedRepoData;
const abandoned = abandonedFixture as unknown as NormalizedRepoData;
const singleMaintainer = singleMaintainerFixture as unknown as NormalizedRepoData;
const criticalCve = criticalCveFixture as unknown as NormalizedRepoData;
...
describe('computeScore — output shape', () => {
  ...
  it('all dimension scores are 0–100', () => {
    for (const fixture of [thriving, abandoned, singleMaintainer, criticalCve]) {
      const result = computeScore(fixture);
      for (const dim of ['maintenance', 'community', 'security', 'releases'] as const) {
