/**
 * Releases Dimension Scorer
 *
 * Pure function — no I/O, no side effects.
 * Measures: release cadence, cadence stability, breaking change rate.
 */
import type { NormalizedReleases, DimensionScoreResult, ScoreBreakdownItem } from '../types.js';
import { RELEASES } from '../config.js';

export function scoreReleases(data: NormalizedReleases): DimensionScoreResult {
  ...
  const cadencePoints = (() => {
    ...
    if (avg <= RELEASES.CADENCE_EXCELLENT_DAYS) return RELEASES.CADENCE_MAX_POINTS;
  ...
  const stabilityPoints = (() => {
    ...
    if (std <= RELEASES.STABILITY_EXCELLENT_STD_DEV) return RELEASES.STABILITY_MAX_POINTS;
  ...
  const breakingPoints = (() => {
    if (data.totalReleases === 0) return Math.round(RELEASES.BREAKING_CHANGE_MAX_POINTS * 0.5);
    const rate = data.majorVersionBumps / data.totalReleases;
    if (rate <= RELEASES.BREAKING_CHANGE_EXCELLENT_RATE) return RELEASES.BREAKING_CHANGE_MAX_POINTS;
