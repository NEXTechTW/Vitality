...
export function renderReport(result: ScoringEngineOutput): string {
  const lines: string[] = [];
  ...
  lines.push('');
  lines.push(
    `  ${bar(result.score)} ${scoreColor(result.score).bold(String(result.score))}${chalk.gray('/100')}  ${chalk.dim(`[${grade(result.score)}]`)}`,
  );
  lines.push('');
  ...
  for (const [label, score] of dims) {
    lines.push(
      `  ${chalk.dim(label)}  ${bar(score)}  ${scoreColor(score)(String(score).padStart(3))}`,
    );
  }
...
export function renderBreakdown(result: ScoringEngineOutput): string {
  const lines: string[] = [];
