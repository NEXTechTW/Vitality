...
function giniDiversity(commitCounts: number[]): number {
  if (commitCounts.length === 0) return 0;
  if (commitCounts.length === 1) return 0;
...
export function normalize(raw: RawRepoData): NormalizedRepoData {
  ...
  const securityReleases = raw.releases.filter((r) =>
    /security|cve|vuln|patch/i.test(r.name ?? '') || /security|cve|vuln/i.test(r.descriptionHTML ?? ''),
  );
