 ...
 * Free public API — no key required: https://api.osv.dev
 */
import type { RawDependency, RawVulnerability } from '../types.js';
...
function ecosystemToOsv(eco: RawDependency['ecosystem']): string {
...
function parseCvss(vulns: OsvVuln[]): number {
  let max = 0;
...
function cvssToSeverity(score: number): RawVulnerability['severity'] {
...
export async function collectVulnerabilities(
  ...
  for (let i = 0; i < directDeps.length; i += batchSize) {
    ...
    if (!res.ok) continue; // Don't fail the whole analysis on OSV errors

    const data = (await res.json()) as { results: OsvQueryResponse[] };
    for (let j = 0; j < (data.results?.length ?? 0); j++) {
      const result = data.results[j];
      const dep = batch[j];
      if (!result?.vulns?.length || !dep) continue;
