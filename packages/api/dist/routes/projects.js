...
export const projectRoutes = async (app) => {
    ...
    app.get('/:owner/:repo', async (req, reply) => {
      ...
      `;
        ...
        const result = { ...rows[0]?.report, fetched_at: rows[0]?.created_at };
    ...
    app.post('/:owner/:repo/analyze', async (req, reply) => {
        const { owner, repo } = req.params;
        // Cooldown check
    ...
    app.get('/:owner/:repo/history', async (req, reply) => {
        const { owner, repo } = req.params;
...
async function runAnalysis(app, owner, repo, jobId) {
    try {
        await app.sql `
